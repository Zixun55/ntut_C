...
typedef struct{
    process_t process[SIZE];
    int processNum;
    int completeNum;
} work_t;

int min(int machineTimes[], work_t work){
    int machineTime = machineTimes[work.process[work.completeNum].machineNum];
    int workTime = 0;

    if (work.completeNum != 0) workTime = work.process[work.completeNum - 1].workTime;

    return (machineTime > workTime ? machineTime : workTime) + work.process[work.completeNum].demandTime;
}

int main(){
    int n, m, totalProcessNum = 0, minTime, timeTmp, indexTmp, ans = 0;

    scanf("%d %d%*c", &n, &m);
    int machineTimes[n];
    for (int i = 0; i < n; i++) machineTimes[i] = 0;
    work_t works[m];
    for (int i = 0; i < m; i++){
        scanf("%d%*c", &works[i].processNum);
        totalProcessNum += works[i].processNum;
        works[i].completeNum = 0;
        for (int j = 0; j < works[i].processNum; j++)
            scanf("%d %d", &works[i].process[j].machineNum, &works[i].process[j].demandTime);
    }

    for (int i = 0; i < totalProcessNum; i++){
        minTime = __INT_MAX__;
        for (int j = 0; j < m; j++){
            if (works[j].completeNum == works[j].processNum) continue;
            timeTmp = min(machineTimes, works[j]);
            if (timeTmp < minTime){
                minTime = timeTmp;
                indexTmp = j;
            }
        }
        machineTimes[works[indexTmp].process[works[indexTmp].completeNum].machineNum] = minTime;
        works[indexTmp].process[works[indexTmp].completeNum].workTime = minTime;
        works[indexTmp].completeNum++;
    }
    for (int i = 0; i < m; i++) ans += works[i].process[works[i].processNum - 1].workTime;
